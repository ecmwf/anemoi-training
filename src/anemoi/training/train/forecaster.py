...
class GraphForecaster(pl.LightningModule):
    ...
    def get_loss_function(
        ...
        **kwargs: Union[torch.Tensor, tuple[Union[int, tuple[int]], torch.Tensor]],  # noqa: FA100
        ...
        kwargs:
            Kwargs to add to loss function

        If a scalar is to be added to the loss, ensure it is in `scalars` in the loss config

        E.g.
            If `scalars: ['variable_scaling']` is set in the config, and `variable_scaling` in `scalars`
             `variable_scaling` will be added to the scalar of the loss function.
