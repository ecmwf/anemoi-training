...
class LongRolloutPlots(BasePlotCallback):
    ...
    def _plot(
        ...
        # batch = pl_module.model.pre_processors(batch, in_place=False) # after update to data indices
        # prepare input tensor for rollout from preprocessed batch
        x = batch[
            :,
            0 : pl_module.multi_step,
            ...,
            pl_module.data_indices.data.input.full,  # internal_data
        ]  # (bs, multi_step, latlon, nvar)
        assert (
        ...
        input_tensor_0 = batch[
            ...
            ...,
            pl_module.data_indices.data.output.full,  # internal_data
        ].cpu()
